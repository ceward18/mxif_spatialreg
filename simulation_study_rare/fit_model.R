...
fit_model <- function(model_type, sim_data, model_info, beta_val, beta2) {
    ...
    if (model_type == 'no_corr') {
        ...
        fov_dat <- aggregate(outcome ~ imageID, data = sim_data, sum)
        
        # eigen-decomposition models
        ...
        vario_dat <- cbind.data.frame(imageID = unique_images, vario_ests)
        vario_dat <- merge(vario_dat, fov_dat, by = 'imageID')
        vario_dat <- vario_dat[vario_dat$outcome > 0,]
        
        sigma_spat_use <- sqrt(mean(vario_dat$sigma_spat))
        # remove those with spatial range bigger than maximum distance between cells
        zero_dist_use <- mean(vario_dat$zero_distance[vario_dat$zero_distance < 1060.66])
